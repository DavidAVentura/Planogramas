...
sequenceDiagram
    ...
    %% Índices únicos filtrados garantizan una sola versión por planograma+tipo en cada uno de
    %% los 4 estados no archivados (borrador/en_desarrollo/piloto/publicado). Cada promoción de
    %% estado archiva automáticamente a la versión que ocupaba el estado destino.
    %% ────────────────────────────────────────────────────────────────────────────
    ...
    rect rgba(89, 89, 89, 1)
        ...
        alt Planograma archivado
            ...
            API->>DB: SELECT COUNT(*) FROM PlanogramaVersion<br/>WHERE planograma_id=@planogramaId AND tipo=@tipo<br/>AND estado = 'borrador'
            DB-->>API: countBorrador

            alt Ya existe versión en borrador del mismo tipo
