...
sequenceDiagram
    ...
    %% Índice único filtrado garantiza una sola versión 'publicado' por planograma+tipo.
    %% ────────────────────────────────────────────────────────────────────────────
    ...
    rect rgba(89, 89, 89, 1)
        ...
        alt Planograma archivado
            ...
            API->>DB: SELECT COUNT(*) FROM PlanogramaVersion<br/>WHERE planograma_id=@planogramaId AND tipo=@tipo<br/>AND estado IN ('borrador','en_desarrollo','piloto')
            DB-->>API: countActivas

            alt Ya existe versión activa del mismo tipo
                API-->>FE: 409 Conflict<br/>{ error: "Ya existe una versión activa de tipo {tipo}.<br/>Archívala antes de crear una nueva." }
                FE-->>Analista: Muestra error
            else
